...
open class BlurView: UIView {
    @ObservedObject private var viewModel: BlurViewModel
    ...
    private func setup() {
        guard let viewToAdd = blurController.view else {
            return
        }
...
private class BlurViewModel: ObservableObject {
    ...
    @Published var opaqueEnabled: Bool = false
}
...
private struct BlurredView<Content: View>: View {
    ...
    @ObservedObject var viewModel: BlurViewModel
